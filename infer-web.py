...
def extract_f0_feature(gpus, n_p, f0method, if_f0, exp_dir, version19, gpus_rmvpe, echl, spk_id5):
    logger.info(f"Received spk_id5: {spk_id5}")
    print(f"Received spk_id5: {spk_id5}")
    ...
    # Ensure exp_dir includes sid_{spk_id5} only once
    exp_dir = os.path.join(exp_dir, f"sid_{spk_id5}")
    exp_dir = os.path.normpath(exp_dir)  # Normalize path

    # Construct the full log directory path (ensure that now_dir is set correctly and absolute)
    now_dir = os.path.abspath(now_dir)  # Ensure now_dir is absolute
        ...
        with gr.TabItem("Training"):
            ...
            with gr.Group():  # 暂时单人的, 后面支持最多4人的#数据处理
                ...
                with gr.Row():
                    ...
                    # speaker_name = gr.Textbox(label="Speaker name:", value="Speaker1")
                    spk_id5 = gr.Textbox( #gr.Slider
                        #minimum=0,
                        #maximum=108,
                        label= " Speaker ID - Experimental ", # "Speaker/Singer ID:   Multi-speaker training is currently unsupported.",
                        value=0,
                        interactive=True,
                        visible=True,
                    )
...
try:
    ...
    if config.global_link:
        ...
        app.queue(max_size=1022).launch(
            ...
            server_port=7897, # config.listen_port
            quiet=True,
        )
