...
class Synthesizer(torch.nn.Module):
    ...
    def forward(
        ...
        if y is not None:
            ...
            #print("        /// Synth Forward /// Shape of 'spec' tensor aka. y:", y.shape)  # This will print the full shape: [batch_size, num_channels, sequence_length]
            #sequence_length = y.size(2)  # The sequence length is the third dimension
            #print("        /// Synth Forward /// Sequence length of y (before slicing):", sequence_length)  # This will print the sequence length

            z, m_q, logs_q, y_mask = self.enc_q(y, y_lengths, g=g)
            z_p = self.flow(z, y_mask, g=g)
            z_slice, ids_slice = rand_slice_segments(z, y_lengths, self.segment_size)
            
            #print("        /// Synthesizer Forward /// z_slice shape after ids_slice:", z_slice.shape)
