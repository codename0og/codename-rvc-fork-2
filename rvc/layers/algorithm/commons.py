...
def collect_all_slices(tensor: torch.Tensor, lengths: torch.Tensor, segment_size: int, step_size: Optional[int] = 1):
    """
    ...
    # Convert lengths to tensor if it's a tuple
    if isinstance(lengths, tuple):
        lengths = torch.tensor(lengths)

    for i in range(len(lengths)):
        seq_length = lengths[i].item()
        # Slice the tensor into segments of size segment_size
        for start in range(0, seq_length, segment_size):
            end = min(start + segment_size, seq_length)
            slices.append(tensor[i, start:end])  # Collect slice
            ids.append((i, start, end))  # Store the slice indices
