...
from rvc.layers.algorithm.residuals import LRELU_SLOPE, ResBlock1, ResBlock2 #
...
class Generator(torch.nn.Module):
    ...
    def __init__(
        ...
        for i, (u, k) in enumerate(zip(upsample_rates, upsample_kernel_sizes)):
            # Calculate output padding and adjusted padding
            op = (k - u) % 2  # Output padding is 1 if remainder exists, 0 otherwise
            p = (k - u + op) // 2  # Adjusted padding to center kernel properly
            ...
            self.ups.append(
                weight_norm(
                    torch.nn.ConvTranspose1d(
                        ...
                        u,
                        padding=p,
                        output_padding=op  # Add the computed output padding
                    )
                )
            )
