...
from rvc.layers.discriminators.sub.mpd import MultiPeriodDiscriminator
...
def run(
    ...
    MPD = MultiPeriodDiscriminator(use_spectral_norm)
    CQT = MultiScaleSubbandCQTDiscriminator(filters=64, sample_rate=sample_rate)
    STFT = MultiScaleSTFTDiscriminator(filters=64)
    ...
    optim_g = Ranger( # torch.optim.RAdam(
        net_g.parameters(),
        lr = 1e-4,
        betas = (0.9, 0.999),
        eps = 1e-8,
        ...
        gc_conv_only=True,
        gc_loc=False,
        #decoupled_weight_decay  = False, # FOR RAdam
        #foreach = False, # FOR RAdam
    )
    optim_d = Ranger( # torch.optim.RAdam(
        itertools.chain(net_d_mpd.parameters(), net_d_cqt.parameters(), net_d_stft.parameters()),
        lr = 1e-4,
        betas = (0.9, 0.999),
        eps = 1e-8,
        weight_decay = 0,
    ...
    try:
        ...
        _, _, _, epoch_str = load_checkpoint(
            latest_checkpoint_path(experiment_dir, "D_*.pth"), [net_d_mpd, net_d_cqt, net_d_stft], optim_d
        )
...
#    hf_loss = HighFrequencyArtifactLoss(config) # Loss function targetting High-frequency related artifacts ( Which mostly affect sibilants like "s", "c" ) - EXPERIMENTAL
    ...
    with tqdm(total=len(train_loader), leave=False) as pbar:
        for batch_idx, info in data_iterator:
            ...
            # debug
            #print(f"    /// LoaderMultiNSFsid /// Batch {batch_idx}:")
            #print(f"    /// LoaderMultiNSFsid /// phone length: {phone.shape[1]} frames")
            #print(f"    /// LoaderMultiNSFsid /// pitch length: {pitch.shape[1]} frames")
            #print(f"    /// LoaderMultiNSFsid /// pitchf length: {pitchf.shape[1]} frames")
            #print(f"    /// LoaderMultiNSFsid /// spec length: {spec.shape[2]} frames")
            #print(f"    /// LoaderMultiNSFsid /// wave length: {wave.shape[2]} samples")

            # debug
            #print(f"    /// LoaderMultiNSFsid /// phone_lengths: {phone_lengths}")
            #print(f"    /// LoaderMultiNSFsid /// spec_lengths: {spec_lengths}")
            #print(f"    /// LoaderMultiNSFsid /// wave_lengths: {wave_lengths}")
            ...
            with autocast(enabled=use_amp): # with autocast(enabled=use_amp, dtype=torch.float16 or torch.bfloat16):
                (
                    y_hat,
                    ids_slice,
                    x_mask,
        ...
        # --- Discriminators Update ---
            ...
            grad_norm_d_mpd = torch.nn.utils.clip_grad_norm_(net_d_mpd.parameters(), max_norm=1000.0)    #
            grad_norm_d_cqt = torch.nn.utils.clip_grad_norm_(net_d_cqt.parameters(), max_norm=1000.0)    #
            grad_norm_d_stft = torch.nn.utils.clip_grad_norm_(net_d_stft.parameters(), max_norm=1000.0)  #

        # Nan and Inf debugging:

            # for MPD
            if math.isnan(grad_norm_d_mpd):
                print('grad_norm_d_mpd is NaN')
            elif math.isinf(grad_norm_d_mpd):
                print('grad_norm_d_mpd is Inf')

            # for CQT
            if math.isnan(grad_norm_d_cqt):
                print('grad_norm_d_cqt is NaN')
            elif math.isinf(grad_norm_d_cqt):
                print('grad_norm_d_cqt is Inf')

            # for STFT
            if math.isnan(grad_norm_d_stft):
                print('grad_norm_d_stft is NaN')
            elif math.isinf(grad_norm_d_stft):
                print('grad_norm_d_stft is Inf')
        ...
        # --- Generator Update ---
            ...
            with autocast(enabled=use_amp): # ,dtype=torch.bfloat16 or float16   ( If you want to override dtype

                # Zeroing gradients
                optim_g.zero_grad()

            # MPD Loss:
                y_dm_hat_r, y_dm_hat_g, fmap_m_r, fmap_m_g = net_d_mpd(wave, y_hat)
            # CQT Loss:
                y_dc_hat_r, y_dc_hat_g, fmap_c_r, fmap_c_g = net_d_cqt(wave, y_hat)
            # STFT Loss:
                y_ds_hat_r, y_ds_hat_g, fmap_s_r, fmap_s_g = net_d_stft(wave, y_hat)
                ...
                with autocast(enabled=False): # In full precision FP32
                    ...
                    loss_mel = fn_mel_loss_multiscale(wave, y_hat) * 15 # MEL loss         ( wave = GT / reference, y_hat = predicted / generated )
                    ...
                    loss_gen_s, losses_gen_s = generator_loss(y_ds_hat_g) # Generator's minimax for CQT
...
#            loss_kl = 9
    ...
    if rank == 0:
        ...
        if epoch % save_every_epoch == 0:
            ...
            save_checkpoint(
                [net_d_mpd, net_d_cqt, net_d_stft],
                optim_d,
                config.train.learning_rate,
                epoch,
