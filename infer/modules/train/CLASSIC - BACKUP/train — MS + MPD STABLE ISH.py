...
def run(
    ...
    MultiDisc = [
        #MultiPeriodDiscriminator(use_spectral_norm),
    ...
    # Sets CombinedDiscriminator as 1st Discriminator ( net_d_ms )
    net_d_ms = CombinedDiscriminator(MultiDisc)
    if torch.cuda.is_available():
        net_d_ms = net_d_ms.to(device)

    # Sets MultiPeriodDiscriminator as 2nd Discriminator ( net_d_mpd )
    net_d_mpd = MPD
    if torch.cuda.is_available():
    ...
    optim_d = torch.optim.RAdam(
        itertools.chain(net_d_ms.parameters(), net_d_mpd.parameters()),
        lr = 1e-4,
        betas = (0.9, 0.999),
        eps = 1e-8,
        weight_decay = 0,
    ...
    if n_gpus > 1:
        ...
        net_d_ms = DDP(net_d_ms, device_ids=[rank], find_unused_parameters=True)
    ...
    try:
        ...
        _, _, _, epoch_str = load_checkpoint(
            latest_checkpoint_path(experiment_dir, "D_*.pth"), [net_d_ms, net_d_mpd], optim_d
        )
        ...
        if pretrainD != "":
            ...
            # First goes the:  MultiScale SubBand CQT Discriminator + MultiScale STFT Discriminator combo
            if hasattr(net_d_ms, "module"):
                net_d_ms.module.load_state_dict(state_dicts["model_1"]) # "model_1" because net_d_ms ( 2 'multiscale' disc combo ) is denoted as first discriminator from the list
            else:
                net_d_ms.load_state_dict(state_dicts["model_1"])

            # Second goes the MultiPeriodDiscriminator
            if hasattr(net_d_mpd, "module"):
                net_d_mpd.module.load_state_dict(state_dicts["model_2"]) # "model_2" because net_d_mpd (MPD) is denoted as first discriminator from the list
            else:
                net_d_mpd.load_state_dict(state_dicts["model_2"])
    ...
    with tqdm(total=len(train_loader), leave=False) as pbar:
        for batch_idx, info in data_iterator:
            ...
            with autocast(enabled=use_amp, dtype=torch.bfloat16):
                ...
                # Run the discriminator: MS
                y_ds_hat_r, y_ds_hat_g, _, _ = net_d_ms(wave, y_hat.detach()) # compares segmented 'wave' to generator's prediction 'y_hat'
                with autocast(enabled=False, dtype=torch.bfloat16):
                ...
                loss_disc_all = loss_disc_s + loss_disc_m

            # Discriminator backward and optimization
            scaler.scale(loss_disc_all).backward()
            scaler.unscale_(optim_d)
            grad_norm_d_ms = commons.clip_grad_value(net_d_ms.parameters(), None) #
            grad_norm_d_mpd = commons.clip_grad_value(net_d_mpd.parameters(), None) #

            # Nan and Inf debugging:
            if math.isnan(grad_norm_d_ms) or math.isnan(grad_norm_d_mpd):
                print('grad_norm_d_ms or grad_norm_d_mpd is NaN')
            elif math.isinf(grad_norm_d_ms) or math.isinf(grad_norm_d_mpd):
                print('grad_norm_d_ms or grad_norm_d_mpd is Inf')
            scaler.step(optim_d)

            # --- Generator Update ---

            # Generator backward and update
            with autocast(enabled=use_amp, dtype=torch.bfloat16): # enabled by default
            ...
            # MS Loss:
                y_ds_hat_r, y_ds_hat_g, fmap_s_r, fmap_s_g = net_d_ms(wave, y_hat)
                ...
                # MS:
                    loss_fm_s = feature_loss(fmap_s_r, fmap_s_g) # Feature matching loss for MS
                    #loss_fm = multi_scale_feature_loss_log(fmap_r, fmap_g) # Multi-scale FM loss with logarithmic approach

                    loss_mel = fn_mel_loss_multiscale(wave, y_hat) * 40 # MEL loss         ( wave = GT / reference, y_hat = predicted / generated )
                    ...
                    loss_gen_s, losses_gen_s = generator_loss(y_ds_hat_g) # Generator's minimax for MS

                    # Summed loss of generator:
                    loss_gen_all = loss_gen_s + loss_gen_m + loss_fm_s + loss_fm_m + loss_mel + loss_kl
...
#            loss_kl = 9
        ...
        scalar_dict = {
            ...
            "learning_rate": lr,
            "grad_norm_d_mpd": grad_norm_d_mpd,
            "grad_norm_d_ms": grad_norm_d_ms,
            "loss/g/fm_MPD": loss_fm_m,
            "loss/g/fm_MS": loss_fm_s,
            "loss/g/mel": loss_mel,
            "loss/g/kl": loss_kl,
            #"loss/g/hf": loss_hf,
            #"loss/g/zm": loss_zm,
