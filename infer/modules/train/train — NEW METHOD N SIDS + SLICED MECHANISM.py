...
def run(
    ...
    STFT = MultiScaleSTFTDiscriminator(filters=32)
    ...
    with tqdm(total=len(train_loader), leave=False) as pbar:
        for batch_idx, info in data_iterator:
            ...
            with autocast(enabled=use_amp, dtype=torch.bfloat16):
                (
                    y_hat,  # they are: 36 frames for 17280 segment_size    and 36 frames for 17280 segment_size
                    ids_slice,
                    x_mask,
                    z_mask,
                ...
                y_mel = commons.slice_segments( # Slices / segments the mel spectrograms of groundtruth ( y )
                    mel, #  Mel spectrogram slices / sequences being 36 frames long, made out of 'spec_to_mel_torch'
                    ids_slice,
                    config.train.segment_size // config.data.hop_length, # 36 frames
                    dim=3,
                )
            ...
            if math.isnan(grad_norm_d_mpd) or math.isnan(grad_norm_d_cqt) or math.isnan(grad_norm_d_stft):
                print('grad_norm_d_mpd or grad_norm_d_cqt or grad_norm_d_stft is NaN')
            elif math.isinf(grad_norm_d_mpd) or math.isinf(grad_norm_d_cqt) or math.isinf(grad_norm_d_stft):
                print('grad_norm_d_mpd or grad_norm_d_cqt or grad_norm_stft is Inf')
            scaler.step(optim_d)
