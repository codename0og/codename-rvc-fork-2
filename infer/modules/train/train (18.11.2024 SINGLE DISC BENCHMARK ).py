...
def run(
    ...
    CQT = MultiScaleSubbandCQTDiscriminator(filters=32, sample_rate=sample_rate)
...
#        )
        ...
        _, _, _, epoch_str = load_checkpoint(
            latest_checkpoint_path(experiment_dir, "D_*.pth"), [net_d_cqt], [optim_d_cqt]
        )
    ...
    for epoch in range(epoch_str, total_epoch + 1):
        if rank == 0:
            train_and_evaluate(
                ...
                epoch,
                config,
                [net_g, net_d_cqt], # [net_g, net_d_mpd, net_d_cqt, net_d_stft],
                [optim_g, optim_d_cqt], # [optim_g, optim_d_mpd, optim_d_cqt, optim_d_stft],
                scaler,
                [train_loader, None],
                [writer, writer_eval],
                cache,
...
def train_and_evaluate(
    ...
    if epoch == 1:
        pass
    net_g, net_d_cqt = nets #     net_g, net_d_mpd, net_d_cqt, net_d_stft = nets
    optim_g, optim_d_cqt = optims #     optim_g, optim_d_mpd, optim_d_cqt, optim_d_stft = optims
...
#                    )
                ...
                loss_disc_all = loss_disc_c # loss_disc_m + loss_disc_c + loss_disc_s
...
#                    loss_gen_s, losses_gen_s = generator_loss(y_ds_hat_g) # Generator's minimax for CQT
                    ...
                    loss_gen_all = loss_gen_c + loss_fm_c + loss_mel + loss_kl
...
#            "loss/g/fm_STFT": loss_fm_s,
    ...
    if rank == 0:
        ...
        if epoch % save_every_epoch == 0:
            ...
            save_checkpoint(
                [net_d_cqt], #                 [net_d_mpd, net_d_cqt, net_d_stft],
                [optim_d_cqt], #                 [optim_d_mpd, optim_d_cqt, optim_d_stft],
                [lr_d_cqt], #                 [lr_d_mpd, lr_d_cqt, lr_d_stft],
                epoch,
