...
# ///////////////////////////////////////////////////////////
    ...
    if not original_rvc_training: # original_rvc_training = False
        optim_d_cqt = Ranger(
    ...
    try:
        ...
        if not original_rvc_training:
            ...
            _, _, _, epoch_str = load_checkpoint(
                latest_checkpoint_path(experiment_dir, "D_*.pth"), [net_d_mpd], [optim_d_mpd]
            )
    ...
    for epoch in range(epoch_str, total_epoch + 1):
        if rank == 0:
            train_and_evaluate(
                ...
                epoch,
                config,
                [net_g, net_d_mpd, net_d_cqt, net_d_stft] if not original_rvc_training else [net_g, net_d_mpd],
                [optim_g, optim_d_mpd, optim_d_cqt, optim_d_stft] if not original_rvc_training else [optim_g, optim_d_mpd],
                scaler,
                [train_loader, None],
                [writer, writer_eval],
                cache,
...
def train_and_evaluate(
    ...
    if not original_rvc_training:
        ...
        net_g, net_d_mpd = nets
        optim_g, optim_d_mpd = optims
...
#    hf_loss = HighFrequencyArtifactLoss(config) # Loss function targetting High-frequency related artifacts ( Which mostly affect sibilants like "s", "c" ) - EXPERIMENTAL
    ...
    with tqdm(total=len(train_loader), leave=False) as pbar:
        ...
        # Nan and Inf debugging:
            ...
            # Generator backward
            with autocast(enabled=use_amp): # (enabled=use_amp, dtype=torch.bfloat16)  - If you want to override amp's default 'fp16' with 'bf16'
            ...
            # Loss functions for Generator:
                with autocast(enabled=False): # In full precision  ( FP32 )
                    ...
                    if not original_rvc_training:
                        ...
                        loss_gen_all = loss_gen_m + loss_fm_m + loss_mel + loss_kl
...
#            loss_kl = 9
    ...
    if rank == 0:
        ...
        if epoch % save_every_epoch == 0:
            ...
            save_checkpoint(
                [net_d_mpd, net_d_cqt, net_d_stft] if not original_rvc_training else [net_d_mpd],
                [optim_d_mpd, optim_d_cqt, optim_d_stft] if not original_rvc_training else [optim_d_mpd],
                [lr_d_mpd, lr_d_cqt, lr_d_stft] if not original_rvc_training else [lr_d_mpd],
                epoch,
